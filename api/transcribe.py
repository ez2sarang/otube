...
def image_hash(path: str, hash_size: int = 8) -> str:
    """간이 이미지 해시 (Average Hash). 비슷한 프레임 비교용."""
...
def extract_unique_text_frames(
    video_path: str,
    output_dir: str,
    ...
    """영상에서 화면이 바뀌는 지점의 프레임을 추출하고, 중복을 제거.

    1. ffmpeg scene detection으로 장면 전환점 탐지
    2. 각 전환점에서 프레임 추출
    3. Average Hash로 비슷한 프레임 중복 제거
    4. 고유 프레임만 반환
    """
    os.makedirs(output_dir, exist_ok=True)
    ...
    # max_frames 제한 (추출 전)
    if len(timestamps) > max_frames * 2:
        step = len(timestamps) / (max_frames * 2)
        timestamps = [timestamps[int(i * step)] for i in range(max_frames * 2)]

    # 2. 프레임 추출
    raw_frames = []
    for i, ts in enumerate(timestamps):
        out_path = os.path.join(output_dir, f"raw_{i:04d}.jpg")
        subprocess.run(
    ...
    # 3. 중복 제거 (Average Hash)
    unique_frames = []
    seen_hashes = []

    for frame in raw_frames:
        try:
            h = image_hash(frame["path"])
        except Exception:
            continue

        is_duplicate = False
        for seen_h in seen_hashes:
            if hash_distance(h, seen_h) < similarity_threshold:
                is_duplicate = True
                break
        ...
        if not is_duplicate:
            ...
            final_path = os.path.join(output_dir, f"frame_{idx:04d}.jpg")
            os.rename(frame["path"], final_path)
    ...
    # 남은 raw 파일 정리
    for f in glob.glob(os.path.join(output_dir, "raw_*.jpg")):
        os.remove(f)
